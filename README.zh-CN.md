...
影像增强（降噪、去 JPEG 伪影、超分）**尚未在当前预览版开放使用**。Windows 构建已接入 `enhance-worker`，共用随包 ONNX Runtime；但五档模型仍缺少获准分发的 ONNX 资产、许可归属材料和固定下载清单，因此下载与执行保持关闭。安装后的发行版成功增强和前后对比预览尚未验证。所需资料见[模型交付要求](docs/enhance-model-delivery.md)。

2026-09-22 的隔离 Windows 发行构建已通过 MSI/NSIS 内容检查，以及 NSIS 安装后的 worker 启动、ONNX Runtime 初始化、模型文件缺失拒绝和不可用状态的界面/IPC 门控验收。这些拒绝路径下的测试原图字节保持不变；成功处理路径仍待验收。
...
“功能与插件”页面统一管理官方版永久授权：一次激活覆盖图片高级编辑、OCR、影像增强和 PSD 引擎。RAW 与视频格式扩展免费。插件安装、模型准备与授权分别显示；卸载插件保留授权，移除本机授权保留原文件、模型和插件。

产品尚未开放购买，正式商品页、生产公钥与发行资料仍待交付；影像增强还缺发行模型，激活不代表当前已可使用。本轮完成局部测试及浏览器模拟界面验证，未完成新安装包的端到端使用验收。

...
/// 去重候选的 keyset 游标，按 `(file_size,item_id)` 排序。文件夹候选/树查询另有
/// `DuplicateFolderCandidateCursor`/`DuplicateFolderTreeCursor`，故意分成不同类型，
/// 防止调用方混用游标。
...
/// `push_eligible_predicates` 同源，仅保留组规模 ≥ 2 的组。
